...
# TODO: handle nmap crash, test with "--top-ports 10000"
...
class Host:
    ...
    def to_rows(self) -> list[list[str]]:
        ...
        for port in self.ports.values():
            # [str(v) for k, v in self.to_dict().items() if k in self.HEADER] + list(self.infos.values())
...
def main() -> None:
    ...
    output_thread = Thread(target=parse_output, args=(process.stdout, printer))
    output_thread.start()
    ...
    with progress as progress:
        ...
        while not progress.finished:
            with completed_counter_lock, target_counter_lock:
                progress.update(taskid, advance=completed_counter)
            time.sleep(1)
    ...
    if rc != 0:
        ...
        print(f'nmap exited with code {rc}', file=sys.stderr)
    exit(rc)


def parse_output(reader: TextIO, printer: Printer) -> None:
    global completed_counter_lock
    global completed_counter
    ...
    for event, element in ElementTree.iterparse(reader, events=['start', 'end']):
        ...
        if event == 'start' and element.tag == 'nmaprun':
            if not element.attrib.get('xmloutputversion', '').startswith('1.'):
                print('unsupported xml schema version', file=sys.stderr)
                exit(1)
